...
- **Administration plateforme** (TechNova, [ADR-0031](../adr/0031-console-technova.md)) :
  ...
  changement de plan, double audit (plateforme + journal de l'entreprise). Depuis 3.3-A :
  décision (confirmation / rejet définitifs) des paiements d'abonnement déclarés par les
  entreprises, sans activation ([ADR-0032](../adr/0032-paiements-abonnement.md)). Restriction
...
- Caisse : sessions (ouverture / fond de caisse / mouvements / fermeture / comptage et
  écart) — **réalisée en Phase 2.9** ([`CASH_REGISTER.md`](CASH_REGISTER.md), ADR-0022) :
  caisse d'un site, une seule session ouverte par caisse, solde calculé à partir des
  mouvements (append-only), paiement espèces et mouvement de caisse dans la même transaction.
...
| Abonnement | Chaîne **paiement → licence `.lic` → activation** (paiements `PENDING`/`CONFIRMED`/`REJECTED` confirmés par TechNova, licence signée par la clé privée TechNova et vérifiée par clé publique) — **3.3-A** paiements ✅ (ADR-0032 ; un paiement confirmé n'active rien), **3.3-B** licences ; remplacera l'activation manuelle transitoire de la console (3.2-G) | ADR-0025, ADR-0031 |
